var CanvasParticle = (function() {
    ...
    function canvasInit(canvasConfig) {
        ...
        var canvas = {
            ...
            // 默认配置
            config: {
                vx: canvasConfig.vx || 10,
                vy: canvasConfig.vy || 10,
                height: canvasConfig.height || 5,
                width: canvasConfig.width || 5,
                count: canvasConfig.count || 500,
                color: canvasConfig.color || "0, 128, 255",
                stroke: canvasConfig.stroke || "0,38,77",
        ...
        for (var i = 0, len = canvas.config.count; i < len; i++) {
            ...
            for (var j = 0; j < len; j++) {
                if (i != j) {
                    ...
                    if (dist <= canvas.config.dist + 5000 && canvas.points[i].max_conn < canvas.config.max_conn) {
                        canvas.points[i].max_conn++;
                        // 距离越远，线条越细，而且越透明
                        context.lineWidth = 1.0 - dist / canvas.config.dist;
            ...
            // 如果鼠标进入画布
            // point to mouse
            var leftss = document.getElementsByClassName("login")[0].offsetLeft - 30;
            var topss = document.getElementsByClassName("login")[0].offsetTop - 30;
            var widthss = document.getElementsByClassName("login")[0].offsetWidth + 30;
            var heightss = document.getElementsByClassName("login")[0].offsetHeight + 30;
            if (mouse) {
                if ((mouse.x < leftss || mouse.x > leftss + widthss + 30) || (mouse.y < topss || mouse.y > topss + heightss + 30)) {
